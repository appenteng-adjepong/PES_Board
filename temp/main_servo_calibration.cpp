...
int main()
{
    ...
    while (true) {
        ...
        if (do_execute_main_task) {
            ...
            led1 = 1;
            // enable the servos and move them to the zero position
            ...
            servo_D3.setPulseWidth(servo_input);

            // calculate the inputs for the servos for the next cycle
            if ((servo_input < 1.0f) && 
            (servo_counter % loops_per_seconds == 0) &&
            (servo_counter != 0))
                servo_input = (0.5f-servo_input);
            servo_counter ++;
        } else {
